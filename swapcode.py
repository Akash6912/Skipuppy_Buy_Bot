...
class Uniswap:
    def __init__(self, wallet_address, private_key, provider, web3):
        ...
        self.permit2 = self.w3.eth.contract(
            address=Web3.to_checksum_address("0x000000000022D473030F116dDEE9F6B43aC78BA3"), abi=PERMIT2_ABI)
    ...
    def make_trade(self, from_token, to_token, amount, fee, slippage, pool_version="v3"):
        ...
        if balance < amount:
            raise ValueError(
                f"Insufficient balance. Have: {balance / (10 ** decimals_in)}, Need: {amount / (10 ** decimals_in)}")
        ...
        if pool_version.lower() == "v3":
            ...
            encoded_data = (
                ...
                .build_v4_swap()
                .build_transaction(self.account.address, amount_in_wei if is_native_input else 0,
